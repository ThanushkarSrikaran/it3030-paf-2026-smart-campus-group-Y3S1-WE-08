...
const AdminPanel = () => {
    ...
    const cardStyle = { background: 'rgba(10,20,50,0.7)', border: '1px solid rgba(56,189,248,0.12)', backdropFilter: 'blur(12px)' };

    return (
        <div className="p-6 md:p-8 min-h-[calc(100vh-5rem)]" style={{ background: '#000814' }}>
            <div className="max-w-7xl mx-auto space-y-6">
                {/* Header */}
                <div className="rounded-3xl overflow-hidden relative min-h-[160px] flex flex-col justify-center"
                    style={{ background: 'linear-gradient(135deg,#0a0f2e 0%,#0d1b4b 50%,#1a0533 100%)', border: '1px solid rgba(56,189,248,0.12)' }}>
                    <div className="absolute top-0 right-1/4 w-72 h-36 rounded-full pointer-events-none opacity-20"
                        style={{ background: 'radial-gradient(circle,#7c3aed,transparent)', filter: 'blur(30px)' }} />
                    <div className="absolute bottom-0 left-1/4 w-48 h-24 rounded-full pointer-events-none opacity-15"
                        style={{ background: 'radial-gradient(circle,#0ea5e9,transparent)', filter: 'blur(20px)' }} />
                    <div className="relative z-10 px-8 py-8 text-white">
                        <div className="flex items-center gap-3 mb-2">
                            <Shield className="w-8 h-8 text-sky-400" />
                            <h1 className="text-3xl md:text-4xl font-black tracking-tight italic">Admin Control Center</h1>
                        </div>
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
                    {[
                        { label: 'Users', value: overview.totalUsers, Icon: Users, color: '#38bdf8' },
                        { label: 'Staff', value: overview.staffUsers, Icon: Wrench, color: '#818cf8' },
                        { label: 'Tickets', value: overview.totalTickets, Icon: Ticket, color: '#a78bfa' },
                        { label: 'Open', value: overview.openTickets, Icon: Ticket, color: '#fbbf24' },
                        { label: 'In Progress', value: overview.inProgressTickets, Icon: Loader, color: '#34d399' },
                    ].map(({ label, value, Icon, color }) => (
                        <div key={label} className="rounded-2xl p-5 transition-all hover:-translate-y-0.5" style={cardStyle}>
                            <div className="flex items-center justify-between mb-3">
                                <span className="text-xs font-black uppercase tracking-wider text-slate-400">{label}</span>
                                <Icon className="w-4 h-4" style={{ color }} />
                            </div>
                            <p className="text-3xl font-black text-white" style={{ textShadow: `0 0 20px ${color}40` }}>{value}</p>
                        </div>
                    ))}
                </section>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    {/* Onboard Staff Form */}
                    <div className="xl:col-span-1 rounded-[2rem] overflow-hidden flex flex-col relative" style={cardStyle}>
                        <div className="absolute top-0 right-0 w-32 h-32 rounded-full pointer-events-none opacity-20 -mr-8 -mt-8"
                            style={{ background: 'radial-gradient(circle,#7c3aed,transparent)', filter: 'blur(20px)' }} />

                        <div className="px-8 py-7 relative z-10" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                            <div className="flex items-center gap-3 mb-1">
                                <div className="p-2 rounded-xl" style={{ background: 'rgba(56,189,248,0.1)', border: '1px solid rgba(56,189,248,0.2)' }}>
                                    <UserPlus className="w-5 h-5 text-sky-400" />
                                </div>
                                <h2 className="text-xl font-black text-white tracking-tight italic">Onboard Staff</h2>
                            </div>
                            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mt-3 ml-1">Provision Elevated Access</p>
                        </div>

                        <form onSubmit={handleSubmit} className="p-8 space-y-6 flex-1 flex flex-col relative z-10">
                            <div className="space-y-5 flex-1">
                                {[
                                    { label: 'Full Legal Name', name: 'name', type: 'text', placeholder: 'John Doe', Icon: User },
                                    { label: 'Campus Email', name: 'email', type: 'email', placeholder: 'staff@campus.edu', Icon: Mail },
                                    { label: 'Initial Password', name: 'password', type: 'password', placeholder: '••••••••', Icon: Lock },
                                ].map(({ label, name, type, placeholder, Icon }) => (
                                    <div key={name}>
                                        <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 ml-1">{label}</label>
                                        <div className="relative">
                                            <Icon className="w-4 h-4 text-slate-500 absolute left-4 top-1/2 -translate-y-1/2" />
                                            <input type={type} name={name} value={formData[name]} onChange={handleChange} required={name !== 'department'}
                                                minLength={name === 'password' ? 6 : undefined} placeholder={placeholder}
                                                className="w-full pl-11 pr-4 py-3.5 rounded-2xl text-sm font-bold text-white placeholder:text-slate-600 outline-none transition-all"
                            ...
                            <div className="pt-4 mt-auto">
                                <button type="submit" disabled={saving}
                                    className="w-full inline-flex items-center justify-center gap-3 px-7 py-4 rounded-2xl text-white font-black text-xs uppercase tracking-widest disabled:opacity-60 disabled:cursor-not-allowed transition-all"
                                    style={{ background: 'linear-gradient(135deg,#2563eb,#7c3aed)', boxShadow: '0 0 30px rgba(37,99,235,0.3)' }}>
                                    {saving ? <Loader className="w-5 h-5 animate-spin" /> : <Shield className="w-5 h-5" />}
                                    {saving ? 'Authenticating...' : 'Provision Account'}
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div className="xl:col-span-2 flex flex-col">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
                            <div>
                                <h2 className="text-xl font-black text-white tracking-tight italic">User & Staff Directory</h2>
                                <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mt-1 ml-1">Current Staff & Members</p>
                            </div>
                            <div className="relative w-full sm:w-72">
                                <Search className="w-4 h-4 text-slate-500 absolute left-4 top-1/2 -translate-y-1/2" />
                                <input type="text" value={query} onChange={(e) => setQuery(e.target.value)}
                                    placeholder="Search directory..."
                                    className="w-full pl-11 pr-4 py-3 rounded-2xl text-xs font-bold text-white placeholder:text-slate-600 outline-none transition-all"
                                    style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }} />
                            </div>
                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-3 gap-6">
                                            ...
                                            {staffMembers.length === 0 && (
                                                <div className="col-span-full py-10 text-center flex flex-col items-center rounded-3xl" style={{ border: '1px dashed rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.02)' }}>
